...
/**
 * Wraps a GET request so that any failure (403, 404, network error)
 * returns empty data instead of rejecting. This keeps the Reports
 * dashboard functional for users with partial permissions.
 */
const safeGet = async (url, config = {}) => {
    try {
        ...
        // 403 is expected for restricted roles — don't spam the console
        if (status !== 403) {
            console.warn(`[Reports API] Failed to fetch ${url}:`, status || err.message);
        }
        return { data: { data: [] } };
    }
};
...
export const reportsApi = {
    // Dashboard metrics - aggregates from multiple endpoints
    getMetrics: async (params = {}) => {
        try {
            const [itemsRes, requestsRes, ordersRes, deliveriesRes] = await Promise.all([
                safeGet("/items"),
                safeGet("/purchase-requests"),
    ...
    // Inventory analytics - uses /items and /inventory-transactions
