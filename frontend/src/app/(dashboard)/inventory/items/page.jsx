...
const [scannedItem, setScannedItem] = useState(null);
    ...
    const fetchCategories = async () => {
        try {
            ...
            const normalizedResponse = normalizeApiData(
                response.data
            );
    ...
    try {
        // Fetch all existing items to check current barcodes
        const response = await axiosInstance.get("/items");
        const existingItems = response.data?.data || [];
        
        // Extract numbers from existing barcodes
...
const handleScanBarcode = async () => {
    ...
    try {
        ...
        // Second try: if no exact match, search by barcode or name (case-insensitive)
        if (items.length === 0) {
        ...
        if (items.length > 0) {
            ...
            // Barcode not found — allow creation with this barcode
            setForm((prev) => ({
                ...prev,
                barcode: searchValue,
            }));
...
const handleSubmit = async (e) => {
    ...
    try {
        ...
        const payload = {
            ...
            barcode: form.barcode.trim(), // Use the auto-generated barcode
            ...
            {showCategoryModal && (
                ...
                >
                    <div className="w-full max-w-md rounded-xl bg-white shadow-xl max-h-[90vh] flex flex-col">
                        ...
                        <form onSubmit={handleCreateCategory} className="overflow-y-auto p-6 space-y-4 flex-1"
                            style={{
                                scrollbarWidth: 'none',
                                msOverflowStyle: 'none',
                            }}>
